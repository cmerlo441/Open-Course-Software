...
if( $_SESSION[ 'admin' ] == 1 ) {
    ...
    /*
     $class_mail_query = 'select id, subject, message, sent_time '
     . 'from mail_to_classes '
     . "where section = $section "
     . 'order by sent_time, subject';
     $class_mail_result = $db->query( $class_mail_query );
    */

    $student_mail_query = 'select m.id, m.subject, m.message, m.datetime '
	...
	print "You have not sent any e-mail directly to $first.";
    } else {
	print "<div id=\"body\">\n";
	print "<ul>\n";
...
$(document).ready(function(){
    ...
    $('a.open_student_message').click(function(){
        ...
        $.post('display_email_to_student.php',
	    { student: student, section: section, id: id },
	    function(data){
	        $('div#body').html( data );
		$('div#sent_email_dialog').dialog( 'option', 'position', 'center' );
	    }
	)
    })
})
