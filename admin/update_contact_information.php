...
if( $_SESSION[ 'admin' ] == 1 ) {
	...
	if( $update_value != $original_html ) {
		$update_query = "update contact_information set $column = \"$update_value\" "
            . "where $column = \"$original_html\"";
		$update_result = $db->query( $update_query );
		if( $db->affected_rows == 1 ) {
			$new_data_query = "select $column as x from contact_information "
				. "where $column = \"$update_value\"";
