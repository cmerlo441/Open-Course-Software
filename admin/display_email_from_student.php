...
if( $_SESSION[ 'admin' ] == 1 ) {
    ...
    $email_query = 'select * from mail_from_students '
	. "where id = $id";
    ...
    print "Date: " . date( 'l, F j, Y g:i a', strtotime( $email[ 'sent_time' ] ) ) . "<br />\n";
...
$(document).ready(function(){
    ...
    $('div#received_email_dialog').dialog( 'option', 'buttons', {
        'OK': function(){
	...
	"<-- Go Back To List": function(){
	    $.post('list_email_from_student.php',
	        { student: student, section: section },
                function( data ) {
		    $('div#received_email_dialog').html(data).dialog( 'option', 'buttons', {
		        'OK': function(){
	                    $('div#received_email_dialog').dialog('destroy');
	                }
