...
if( $_SESSION[ 'admin' ] == 1 ) {
    ...
    $update_row = $update_result->fetch_object();
    ...
    if( isset( $_POST[ 'update_value' ] ) ) {
        ...
        if( trim( $_POST[ 'update_value' ] ) != trim( $_POST[ 'original_html' ] ) ) {
            ...
            if( $update_row->c == 1 ) {
                ...
                $insert_query = 'insert into semester( name ) '."values( \"$update_value\" )";
            }
        ...
        preg_match( "/^semester_(.*)$/", $db->real_escape_string( $_POST[ 'column' ] ), $matches );
        $column = $matches[ 1 ];
        
        if( $update_row->c == 1 ) {
            $update_query = "update semester set $column = \"".date( 'Y-m-d', strtotime( $db->real_escape_string( $_POST[ 'date' ] ) ) )."\"";
            $update_result = $db->query( $update_query );
            $update_result->close();
        } else {
            $insert_query = "insert into semester ( $column ) "."values( \"".date( 'Y-m-d', strtotime( $db->real_escape_string( $_POST[ 'date' ] ) ) )."\" )";
